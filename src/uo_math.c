...
bool uo_test_matmul()
{
  ...
  {
    float res = uo_dotproduct_ps(test_A, test_Bt, 16);
    float d = test_C_expected[0] - res;
    d = d > 0 ? d : -d;
    passed &= d < 0.01;

    res = uo_dotproduct_ps(test_A, test_Bt + 16, 16);
  ...
  {
    float Bt[8 * 16];
    uo_transpose_ps(test_B, Bt, 16, 8);

    for (size_t i = 0; i < 8 * 16; i++)
    {
      float d = Bt[i] - test_Bt[i];
      d = d > 0 ? d : -d;
      passed &= d < 0.01;
    }

    float C[8 * 8];
    uo_matmul_ps(test_A, Bt, C, 8, 8, 16);
