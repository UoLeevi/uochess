...
bool uo_test_matmul()
{
  ...
  {
    float res = uo_dotproduct_ps(test_A, test_B_t, 16);
    float d = test_C_expected[0] - res;
    d = d > 0 ? d : -d;
    passed &= d < 0.01;

    res = uo_dotproduct_ps(test_A, test_B_t + 16, 16);
    d = test_C_expected[1] - res;
    d = d > 0 ? d : -d;
    passed &= d < 0.01;
  }
  ...
  {
    float B_t[8 * 16];
    uo_transpose_ps(test_B, B_t, 16, 8);

    for (size_t i = 0; i < 8 * 16; i++)
    {
      float d = B_t[i] - test_B_t[i];
      d = d > 0 ? d : -d;
      passed &= d < 0.01;
    }

    float C[8 * 8];
    uo_matmul_ps(test_A, B_t, C, 8, 8, 16);

    for (size_t i = 0; i < 8 * 8; i++)
    {
      float d = test_C_expected[i] - C[i];
      d = d > 0 ? d : -d;
      passed &= d < 0.02;
    }
  }
